...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                ...
                case url.match(/\/users\/\d+$/) && method === 'DELETE':
                    return deleteUser();
                case url.endsWith('/models/save') && method === 'POST':
                    return savemodel();
                case url.endsWith('/models/load/pyspark') && method === 'GET':
                    return loadpyspark();
        ...
        function savemodel() {
            ...
            const model = body;
            if (models.find(x => x.name === model.name)) {
                return error('model ' + model.name + ' already exist');
            }
            models.push(model);
